...
FrekvensPanel::FrekvensPanel(int p_latch, int p_clock, int p_data, int bitDepth,
                             int numPanels) : Adafruit_GFX(16, 16 * numPanels)
{
    init(p_latch, p_clock, p_data, bitDepth, numPanels);
}

FrekvensPanel::FrekvensPanel(int p_latch, int p_clock, int p_data, int numPanels) :
FrekvensPanel(p_latch, p_clock, p_data, 0, numPanels) { }
...
void FrekvensPanel::drawPixel(int16_t x, int16_t y, uint16_t color)
{
    ...
    {   
        if (y >47){
            if (x > 7) { y += 0x40; x &= 0x07; }
            else { y += 0x30;}
        }
        if (y >31){
            if (x > 7) { y += 0x30; x &= 0x07; }
            else { y += 0x20;}
        }
        else if (y >15){
            if (x > 7) { y += 0x20; x &= 0x07; }
            else { y += 0x10;}
        }
        else if (x > 7) { y += 0x10; x &= 0x07; }
        unsigned short address = (x + (y << 3)) & 0xFFF;
        unsigned short ba = address >> 4;
        unsigned short br = address & 0x0F;
        unsigned short ms = (1 << br);
        unsigned short* wp = &buf[ba];
